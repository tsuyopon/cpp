 ...
 * $ g++ client.cc
 *
 * 接続先はhttp://nghttp2.org。ただし、HTTPとして接続している。
 *
 * HTTP2クライアントの軽量実装
 * Reference: https://qiita.com/0xfffffff7/items/0960527dca6de364daeb
 */
#include <iostream>
#include <stdio.h>
#include <stdlib.h>
...
int main(int argc, char **argv)
{
    ...
    int r = (int)::send(_socket, pri.c_str(), pri.length(), (int)NULL);
    ...
    r = (int)::send(_socket, settingframe, BINARY_FRAME_LENGTH, (int)NULL);
    if (r == -1){
        error = get_error();
    ...
    r = (int)::recv(_socket, p, READ_BUF_SIZE, (int)NULL);
    if (r == -1){
        error = get_error();
        ::shutdown(_socket, SD_BOTH);
        close_socket(_socket);
    ...
    r = (int)::send(_socket, settingframeAck, BINARY_FRAME_LENGTH, (int)NULL);
    if (r == -1){
        error = get_error();
        ::shutdown(_socket, SD_BOTH);
        close_socket(_socket);
    ...
    r = (int)::send(_socket, headersframe, 69, (int)NULL);
    if (r == -1){
        error = get_error();
        ::shutdown(_socket, SD_BOTH);
        close_socket(_socket);
    ...
    while (1){
        ...
        r = (int)::recv(_socket, p, BINARY_FRAME_LENGTH, (int)NULL);
        if (r == -1){
            error = get_error();
            ::shutdown(_socket, SD_BOTH);
            close_socket(_socket);
        ...
        if (memcmp(buf, settingframeAck, BINARY_FRAME_LENGTH) == 0){
            ...
            if (frame_type != 1){
                ...
                r = (int)::recv(_socket, p, payload_length, (int)NULL);
                if (r == -1){
                    error = get_error();
                    ::shutdown(_socket, SD_BOTH);
                    close_socket(_socket);
    ...
    r = (int)::send(_socket, goawayframe, 17, (int)NULL);
    if (r == -1){
        error = get_error();
        ::shutdown(_socket, SD_BOTH);
        close_socket(_socket);
